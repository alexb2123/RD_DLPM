...
df = pd.read_excel(open('/Users/alexandrubordei/Downloads/Vendor Pricing Template-27.xlsx','rb'), sheet_name='Main', usecols="B, F, K, L, M")
time = dt.date.today()
...
# Future price change
                if DLPM_cost > last_price or DLPM_cost < last_price and effective_date.floor('D') > time:
                    DLPM_syntax = ['key tab', 'type ' + str(int(store)) + '-' + str(int(item)), 'key tab',
